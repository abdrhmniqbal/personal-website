...
const tabStyles = tv({
  base: [
    'data-hovered:text-fg relative flex cursor-default items-center rounded-full text-sm font-medium whitespace-nowrap outline-hidden transition *:data-[slot=icon]:mr-2 *:data-[slot=icon]:size-4',
...
const Panel = ({ className, ref, ...props }: TabPanelProps) => {
  return (
    <TabPanel
      ...
      className={ctr(
        className,
        'text-fg flex-1 data-focus-visible:outline-hidden',
      )}
    />
  )
}
